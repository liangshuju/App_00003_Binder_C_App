...
struct binder_state *binder_open(size_t mapsize)
{
	...
	if ((ioctl(bs->fd, BINDER_VERSION, &vers)) == -1) {
		fprintf(stderr, "binder : get BINDER_VERSION failed!\n");
		goto fail_open;
	} else {
		if (vers.protocol_version != BINDER_CURRENT_PROTOCOL_VERSION) {
			fprintf(stderr, "binder : driver verion differs form user space!\n");
			goto fail_open;
		}
	}
		...
		struct binder_io *reply, uint32_t target, uint32_t code)
{
	...
	*/
	...
	if (msg->flags & BIO_F_OVERFLOW) {
		fprintf(stderr, "binder_call : txn buffer overflow!\n");
		goto fail;
	}
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
	...
	for (;;) {
		...
		res = ioctl(bs->fd, BINDER_WRITE_READ, &bwr);
		if (res < 0) {
			ALOGE("binder_loop : ioctl faild %s .\n", strerror(errno));
			break;
		}
